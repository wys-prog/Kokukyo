...
void uint128_t::_init_hex(const char *s, std::size_t len) {
  ...
  const std::size_t lower_len =
      (max_len >= double_lower) ? double_lower : max_len;
  const std::size_t upper_len =
      (max_len >= double_lower) ? (max_len - double_lower) : 0;
...
void uint128_t::_init_dec(const char *s, std::size_t len) {
  ...
  for (std::size_t i = 0; *s && ('0' <= *s) && (*s <= '9') && (i < max_len);
       ++s, ++i) {
    *this *= 10;
    *this += *s - '0';
  }
...
void uint128_t::_init_oct(const char *s, std::size_t len) {
  ...
  for (std::size_t i = 0; *s && ('0' <= *s) && (*s <= '7') && (i < max_len);
       ++s, ++i) {
    *this *= 8;
    *this += *s - '0';
  }
...
void uint128_t::_init_bin(const char *s, std::size_t len) {
  ...
  const std::size_t lower_len =
      (max_len >= eight_lower) ? eight_lower : max_len;
  const std::size_t upper_len =
      (max_len >= eight_lower) ? (max_len - eight_lower) : 0;
  s += starting_index;

  for (std::size_t i = 0; *s && ('0' <= *s) && (*s <= '1') && (i < upper_len);
       ++s, ++i) {
    UPPER <<= 1;
    UPPER |= *s - '0';
  }

  for (std::size_t i = 0; *s && ('0' <= *s) && (*s <= '1') && (i < lower_len);
       ++s, ++i) {
    LOWER <<= 1;
    LOWER |= *s - '0';
  }
